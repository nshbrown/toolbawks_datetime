...
DatetimeToolbocks.prototype = {
  ...
  */
  ...
  parseDateString: function(s) {
		var patterns = this._dateParsePatterns;
		for (var x = 0, len = patterns.length; x < len; x++) {
      var re      = patterns[x].re;
      var handler = patterns[x].handler;
      var bits    = re.exec(s);
      if (bits) {
  ...
  magicDate: function() {
    var input = $(this.options.calendarOptions.inputField.id);
    ...
    try {
      ...
      // Human readable date
      if (messageSpan) {
        messageSpan.update(d.toDateString());
